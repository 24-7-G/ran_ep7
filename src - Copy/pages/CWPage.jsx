...
export default function CWPage({ user, isAdmin }) {
  ...
  const [selectedPlayer, setSelectedPlayer] = useState(null), [selectedAttendance, setSelectedAttendance] = useState(null), [auditDetail, setAuditDetail] = useState(null), [showAllNotices, setShowAllNotices] = useState(false);
  ...
  const [noticeMode, setNoticeMode] = useState("new"), [noticePage, setNoticePage] = useState(1), [noticeNewPage, setNoticeNewPage] = useState(1), [noticeOldPage, setNoticeOldPage] = useState(1), [allNoticePage, setAllNoticePage] = useState(1), [noticeSearch, setNoticeSearch] = useState(""), [noticeTypeFilter, setNoticeTypeFilter] = useState("all");
  ...
  const matchingNotices = useMemo(() => notices.filter(n => {
    if (noticeTypeFilter !== "all" && String(n.type || "info") !== noticeTypeFilter) return false;
  ...
  }), [notices, noticeSearchLower, noticeTypeFilter]);
  ...
  const pagedAllNotices = matchingNotices.slice((safeAllNoticePage - 1) * PAGE_SIZE, safeAllNoticePage * PAGE_SIZE);

  useEffect(() => { setNoticePage(1); setNoticeNewPage(1); setNoticeOldPage(1); setAllNoticePage(1) }, [noticeSearch, noticeTypeFilter]);
  ...
  return <div className="cw-page">
    ...
    <section className="cw-notification-top cw-notification-board">
      ...
      <div className="cw-notification-toolbar">
        ...
        <select value={noticeTypeFilter} onChange={e => setNoticeTypeFilter(e.target.value)} aria-label="Filter notifications by type">
        ...
        <button className="cw-btn" onClick={openAllNotifications}>VIEW ALL NOTIFICATIONS ›</button>
      </div>
    ...
    {showAllNotices && <section className="cw-panel cw-notifications-full" id="cw-notifications-full">
      <div className="cw-section-head">
        <div>
          <div className="cw-kicker">GUILD NOTIFICATIONS</div>
          <h2>CW + Treasury Activity &amp; Notifications</h2>
          <p>Complete audit history. NEW is based on the current local calendar day.</p>
        </div>
        <button className="cw-btn" onClick={() => setShowAllNotices(false)}>HIDE NOTIFICATIONS</button>
      </div>
      <div className="cw-notifications-toolbar-full">
        <input value={noticeSearch} onChange={e => setNoticeSearch(e.target.value)} placeholder="Search player, action, salary, item..." />
        <select value={noticeTypeFilter} onChange={e => setNoticeTypeFilter(e.target.value)}>
          <option value="all">ALL TYPES</option><option value="success">ACTIVITY</option><option value="warning">WARNING</option><option value="error">ERROR</option><option value="reward">REWARD</option><option value="info">NOTICE</option>
        </select>
      </div>
      <div className="cw-notifications-results-head"><strong>{matchingNotices.length}</strong><span>{matchingNotices.length ? `${(safeAllNoticePage - 1) * PAGE_SIZE + 1}-${Math.min(safeAllNoticePage * PAGE_SIZE, matchingNotices.length)} shown` : "0 shown"}</span></div>
      <div className="cw-table-scroll cw-notifications-full-table-scroll">
        <table className="cw-table cw-notifications-full-table">
          <thead><tr><th>#</th><th>STATUS</th><th>TYPE</th><th>MESSAGE</th><th>DATE &amp; TIME</th><th>BY</th></tr></thead>
          <tbody>
            {pagedAllNotices.map((n, index) => {
              const isNew = dateKey(n.createdAt || n.timestamp, resolvedTimezone) === noticeDateKey; return <tr key={n.id} className="cw-click-row" onClick={() => setAuditDetail(n)} title="Click to view full notification details">
                <td>{(safeAllNoticePage - 1) * PAGE_SIZE + index + 1}</td>
                <td><span className={`cw-notice-status ${isNew ? "new" : "old"}`}>{isNew ? "NEW" : "OLD"}</span></td>
                <td><span className={`cw-notice-type-icon ${clean(n.module) === "treasury" ? "treasury" : ""}`}>{clean(n.module) === "treasury" ? "₲" : "⚔"}</span></td>
                <td><div className="cw-notification-table-message"><strong>{clean(n.title) || "Activity"}</strong><span>{clean(n.message) || "Record changed."}</span></div></td>
                <td>{formatDateTime(n.createdAt || n.timestamp, resolvedTimezone)}</td>
                <td>{clean(n.createdBy) || "System"}</td>
              </tr>
            })}
            {!pagedAllNotices.length && <tr><td colSpan="6" className="cw-empty">No notifications match your filters.</td></tr>}
          </tbody>
        </table>
      </div>
      <div className="cw-pagination cw-notifications-pagination">
        ...
        <span>{safeAllNoticePage} / {allNoticePageCount}</span>
