...
const CATEGORY_ALIASES = {
  "BOSS HUNT": "BH ATTENDANCE",
  "CLAN WAR": "CW ATTENDANCE",
...
function categoryLabel(value) {
  ...
  return CATEGORY_ALIASES[raw] || (CATEGORIES.includes(raw) ? raw : "OTHER");
}
...
export default function TicketPage({ user, isAdmin }) {
  ...
  useEffect(() => {
    setPage(1);
  }, [view, search, ignFilter, categoryFilter, priorityFilter, sort]);
  ...
  const visibleTickets = useMemo(() => {
    ...
    const result = tickets.filter((ticket) => {
      ...
      if (categoryFilter !== "all" && categoryLabel(ticket.category) !== categoryFilter) return false;
    ...
    return result;
  }, [tickets, view, ignFilter, categoryFilter, priorityFilter, search, sort]);
  ...
  async function submitTicket(event) {
    ...
    try {
      ...
      setModal(null);
      resetForm();
      setMessage(`Ticket submitted for ${ign}. Keep your 6-digit PIN for future edits or deletion.`);
    } catch (error) {
      console.error(error);
    ...
    try {
      ...
      setMessage(
        previousStatus !== nextStatus
          ? `Ticket moved to ${statusInfo(nextStatus).label}.`
          : answer
  ...
  return (
    <div className="ticket-page">
      <section className="ticket-hero">
        <div>
          <div className="ticket-kicker">GUILD SUPPORT • TICKETS</div>
          <h1>{admin ? "Support Ticket Center" : "Guild Support"}</h1>
          <p>
            Create and track tickets for BH attendance, CW attendance, salary, rewards, treasury, inventory, schedules, account issues, and general concerns.
          </p>
        </div>
      ...
      <section className="ticket-board">
        <div className="ticket-board-head">
          <div>
            <div className="ticket-kicker">{admin ? "ADMIN QUEUE" : "MY SUPPORT QUEUE"}</div>
            <h2>{admin ? "Support Ticket Center" : "My Guild Support"}</h2>
            <p>{admin ? "Filter by player, category, priority, status, or search text. Tickets are sorted by latest update." : "Select your registered IGN when creating a ticket. Keep your private 6-digit PIN for ticket changes."}</p>
          </div>
          <div className="ticket-category-chips" aria-label="Ticket categories">
                ...
                <article className="ticket-row" key={ticket.id}>
                  ...
                  <div className="ticket-row-main">
                    <div className="ticket-row-top">
                      ...
                      <span className="ticket-category">{categoryLabel(ticket.category)}</span>
      ...
      {modal === "submit" && (
        <Modal title="CREATE SUPPORT TICKET" onClose={() => setModal(null)} wide>
